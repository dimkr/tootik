...
func (gc *GarbageCollector) Run(ctx context.Context) error {
	...
	if _, err := gc.DB.ExecContext(ctx, `delete from shares where by not in (select id from persons)`); err != nil {
		return fmt.Errorf("failed to remove old shares: %w", err)
	}

	if _, err := gc.DB.ExecContext(ctx, `delete from shares where note not in (select id from notes)`); err != nil {
		return fmt.Errorf("failed to remove old shares: %w", err)
	}
