...
func (gc *GarbageCollector) Run(ctx context.Context) error {
	...
	if _, err := gc.DB.ExecContext(ctx, `delete from notesfts where rowid in (select rowid from notes where inserted < ? and host != ? and not exists (select 1 from bookmarks where bookmarks.note = notes.id))`, now.Add(-gc.Config.NotesTTL).Unix(), gc.Domain); err != nil {
