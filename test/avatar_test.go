...
func TestAvatar_HappyFlow(t *testing.T) {
	...
	assert.Equal(fmt.Sprintf("30 gemini://localhost.localdomain:8443/login/outbox/%s\r\n", strings.TrimPrefix(server.Alice.ID, "https://")), server.Upload("/login/upload/avatar;mime=image/gif;size=63", server.Alice, avatar))
}
...
func TestAvatar_NewUser(t *testing.T) {
	...
	assert.Regexp(`^40 Please wait for \S+\r\n$`, server.Upload("/login/upload/avatar;mime=image/gif;size=63", server.Alice, avatar))
}
...
func TestAvatar_HappyFlowSizeFirst(t *testing.T) {
	...
	server.Alice.Published.Time = server.Alice.Published.Time.Add(-time.Hour)
	assert.Equal(fmt.Sprintf("30 gemini://localhost.localdomain:8443/login/outbox/%s\r\n", strings.TrimPrefix(server.Alice.ID, "https://")), server.Upload("/login/upload/avatar;size=63;mime=image/gif", server.Alice, avatar))
}
...
func TestAvatar_InvalidSize(t *testing.T) {
	...
	server.Alice.Published.Time = server.Alice.Published.Time.Add(-time.Hour)
	assert.Equal("40 Invalid size\r\n", server.Upload("/login/upload/avatar;mime=image/gif;size=abc", server.Alice, avatar))
}
...
func TestAvatar_InvalidType(t *testing.T) {
	...
	server.Alice.Published.Time = server.Alice.Published.Time.Add(-time.Hour)
	assert.Equal("40 Unsupported image type\r\n", server.Upload("/login/upload/avatar;mime=text/plain;size=63", server.Alice, avatar))
}
...
func TestAvatar_NoSize(t *testing.T) {
	...
	server.Alice.Published.Time = server.Alice.Published.Time.Add(-time.Hour)
	assert.Equal("40 Error\r\n", server.Upload("/login/upload/avatar;mime=image/gif;ize=63", server.Alice, avatar))
}
...
func TestAvatar_NoType(t *testing.T) {
	...
	server.Alice.Published.Time = server.Alice.Published.Time.Add(-time.Hour)
	assert.Equal("40 Error\r\n", server.Upload("/login/upload/avatar;mim=image/gif;size=63", server.Alice, avatar))
}
...
func TestAvatar_InvalidImage(t *testing.T) {
	...
	server.Alice.Published.Time = server.Alice.Published.Time.Add(-time.Hour)
	assert.Equal("40 Error\r\n", server.Upload("/login/upload/avatar;mime=image/gif;size=3", server.Alice, []byte("abc")))
}
...
func TestAvatar_TooSmallSize(t *testing.T) {
	...
	server.Alice.Published.Time = server.Alice.Published.Time.Add(-time.Hour)
	assert.Equal("40 Error\r\n", server.Upload("/login/upload/avatar;mime=image/gif;size=10", server.Alice, avatar))
}
...
func TestAvatar_TooBigSize(t *testing.T) {
	...
	server.Alice.Published.Time = server.Alice.Published.Time.Add(-time.Hour)
	assert.Equal("40 Error\r\n", server.Upload("/login/upload/avatar;mime=image/gif;size=64", server.Alice, avatar))
}
