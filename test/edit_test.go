...
func TestEdit_InvalidEscapeSequence(t *testing.T) {
	...
	whisper := server.Handle("/login/whisper?Hello%20world", server.Bob)
	assert.Regexp(`^30 /login/view/\S+`, whisper)
...
func TestEdit_UnauthenticatedUser(t *testing.T) {
	...
	edit := server.Handle(fmt.Sprintf("/login/edit/%s?Hello%%20followers", id), nil)
...
func TestEdit_AddHashtag(t *testing.T) {
	...
	say := server.Handle("/login/say?%23Hello%20world", server.Alice)
	assert.Regexp(`^30 /login/view/\S+`, say)
	...
	edit := server.Handle(fmt.Sprintf("/login/edit/%s?%%23Hello%%20%%23world", id), server.Alice)
	assert.Equal(fmt.Sprintf("30 /login/view/%s\r\n", id), edit)
...
func TestEdit_RemoveHashtag(t *testing.T) {
	...
	say := server.Handle("/login/say?%23Hello%20%23world", server.Alice)
	assert.Regexp(`^30 /login/view/\S+`, say)
	...
	edit := server.Handle(fmt.Sprintf("/login/edit/%s?%%23Hello%%20world", id), server.Alice)
	assert.Equal(fmt.Sprintf("30 /login/view/%s\r\n", id), edit)
...
func TestEdit_KeepHashtags(t *testing.T) {
	...
	edit := server.Handle(fmt.Sprintf("/login/edit/%s?%%23Hello%%20%%20%%23world", id), server.Alice)
	assert.Equal(fmt.Sprintf("30 /login/view/%s\r\n", id), edit)
...
func TestEdit_AddMention(t *testing.T) {
	...
	assert.NotContains(lines, fmt.Sprintf("=> /login/outbox/%s alice", strings.TrimPrefix(server.Alice.ID, "https://")))

	_, err := server.db.Exec("update notes set inserted = inserted - 3600, object = json_set(object, '$.published', ?) where id = 'https://' || ?", time.Now().Add(-time.Hour).Format(time.RFC3339Nano), id)
	assert.NoError(err)

	edit := server.Handle(fmt.Sprintf("/login/edit/%s?Hello%%20%%40alice", id), server.Bob)
	assert.Equal(fmt.Sprintf("30 /login/view/%s\r\n", id), edit)

	lines = strings.Split(server.Handle("/login/view/"+id, server.Alice), "\n")
	assert.NotContains(lines, "> Hello world")
	assert.Contains(lines, "> Hello @alice")
	assert.Contains(lines, fmt.Sprintf("=> /login/outbox/%s alice", strings.TrimPrefix(server.Alice.ID, "https://")))
}
...
func TestEdit_RemoveMention(t *testing.T) {
	...
	say := server.Handle("/login/say?Hello%20%40alice", server.Bob)
	assert.Regexp(`^30 /login/view/\S+`, say)
	...
	edit := server.Handle(fmt.Sprintf("/login/edit/%s?Hello%%20world", id), server.Bob)
	assert.Equal(fmt.Sprintf("30 /login/view/%s\r\n", id), edit)
...
func TestEdit_KeepMention(t *testing.T) {
	...
	edit := server.Handle(fmt.Sprintf("/login/edit/%s?Hello%%20%%20%%40alice", id), server.Bob)
	assert.Equal(fmt.Sprintf("30 /login/view/%s\r\n", id), edit)
...
func TestEdit_PollAddOption(t *testing.T) {
	...
	say := server.Handle("/login/say?%5bPOLL%20So%2c%20polls%20on%20Station%20are%20pretty%20cool%2c%20right%3f%5d%20Nope%20%7c%20Hell%20yeah%21%20", server.Alice)
	assert.Regexp(`^30 /login/view/\S+\r\n$`, say)
	...
	edit := server.Handle(fmt.Sprintf("/login/edit/%s?%%5bPOLL%%20So%%2c%%20polls%%20on%%20Station%%20are%%20pretty%%20cool%%2c%%20right%%3f%%5d%%20Nope%%20%%7c%%20Hell%%20yeah%%21%%20%%7c%%20I%%20couldn%%27t%%20care%%20less", id), server.Alice)
	assert.Equal(fmt.Sprintf("30 /login/view/%s\r\n", id), edit)
...
func TestEdit_RemoveQuestion(t *testing.T) {
	...
	edit := server.Handle(fmt.Sprintf("/login/edit/%s?This%%20is%%20not%%20a%%20poll", id), server.Alice)
	assert.Equal(fmt.Sprintf("30 /login/view/%s\r\n", id), edit)
