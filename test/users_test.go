...
func TestUsers_DM(t *testing.T) {
	...
	dm := server.Handle("/login/dm?Hello%20%40alice", server.Bob)
...
func TestUsers_PublicPostShared(t *testing.T) {
	...
	follow := server.Handle("/login/follow/127.0.0.1/user/erin", server.Alice)
	assert.Equal("30 /login/outbox/127.0.0.1/user/erin\r\n", follow)
...
func TestUsers_PublicPostSharedNotFollowing(t *testing.T) {
	...
	unfollow := server.Handle("/login/unfollow/127.0.0.1/user/erin", server.Alice)
	assert.Equal("30 /login/outbox/127.0.0.1/user/erin\r\n", unfollow)
