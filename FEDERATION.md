...
In addition, tootik randomly (see `CavageDraftFailureThreshold`) rejects incoming, `draft-cavage-http-signatures`-signed `POST` requests from servers that haven't demonstrated RFC9421 support with `401 Unauthorized`, to encourage other servers to retry with RFC9421.
