...
func (a *Activity) UnmarshalJSON(b []byte) error {
	...
	if common.Instrument != nil {
		var instrumentObject Object
		if err := json.Unmarshal(common.Instrument, &instrumentObject); err == nil {
			a.Instrument = &instrumentObject
		} else if err := json.Unmarshal(common.Instrument, &instrumentObject.ID); err == nil {
			a.Instrument = instrumentObject.ID
		} else {
			return ErrInvalidActivity
		}
	}
