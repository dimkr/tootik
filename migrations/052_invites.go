...
func invites(ctx context.Context, domain string, tx *sql.Tx) error {
	if _, err := tx.ExecContext(ctx, `CREATE TABLE invites(code TEXT NOT NULL, certhash TEXT, inviter TEXT NOT NULL, invited TEXT, inserted INTEGER DEFAULT (UNIXEPOCH()))`); err != nil {
		return err
	}
