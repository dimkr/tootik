...
func implements(ctx context.Context, domain string, tx *sql.Tx) error {
	if rows, err := tx.QueryContext(ctx, `SELECT JSON(actor), ed25519privkey FROM persons WHERE ed25519privkey IS NOT NULL AND actor->>'$.type' = 'Application'`); err != nil {
		return err
	} else {
		defer rows.Close()
		...
		for rows.Next() {
			...
			actor.Implements = actor.Generator.Implements
			actor.Generator = ap.Generator{}
