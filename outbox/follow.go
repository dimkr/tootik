	...
	if _, err := tx.ExecContext(
		...
		VALUES
			(
				...
				$2,
				$3,
				(SELECT CASE WHEN host = $4 AND (actor->'$.manuallyApprovesFollowers' IS NULL OR actor->'$.manuallyApprovesFollowers' = JSON('true')) THEN 1 ELSE NULL END FROM persons WHERE id = $2)
			)
		`,
		followID,
		follower.ID,
