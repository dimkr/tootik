...
func (gl *Listener) Handle(ctx context.Context, conn net.Conn) {
	...
	if r.User == nil {
		r.Context = logcontext.Add(ctx, slog.Group("gemini_request", "path", r.URL.Path))
	} else {
		r.Context = logcontext.Add(ctx, slog.Group("gemini_request", "path", r.URL.Path, "user", r.User.PreferredUsername))
	}
