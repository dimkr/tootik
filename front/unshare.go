...
func (h *Handler) unshare(w text.Writer, r *Request, args ...string) {
	...
	if err := h.DB.QueryRowContext(
		r.Context,
		`select activity from outbox where activity->>'$.actor' = $1 and sender = $1 and activity->>'$.type' = 'Announce' and activity->>'$.object' = $2`,
		r.User.ID,
		postID,
	).Scan(&share); err != nil && errors.Is(err, sql.ErrNoRows) {
