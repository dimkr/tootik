...
func NewHandler(domain string, closed bool, cfg *cfg.Config, resolver ap.Resolver, db *sql.DB) (Handler, error) {
	...
	h.handlers[regexp.MustCompile(`^/users/avatar$`)] = withUserMenu(h.avatar)
	h.handlers[regexp.MustCompile(`^/users/avatar/upload;([a-z]+)=([^;]+);([a-z]+)=([^;]+)`)] = h.uploadAvatar
	...
	h.handlers[regexp.MustCompile(`^/users/metadata/remove$`)] = h.metadataRemove
