	...
	h.showFeedPage(
		...
		func(offset int) (*sql.Rows, error) {
			return h.DB.QueryContext(
				r.Context,
				`select notes.object, persons.actor, null, notes.inserted from
				notes
				join
				hashtags
				on
				...
				left join (
					select object->>'$.inReplyTo' as id, count(*) as count from
					notes
					where
				...
				persons
				on
					notes.author = persons.id
				where
					notes.public = 1 and
					hashtags.hashtag = $1
				order by
					replies.count desc,
					notes.inserted/(24*60*60) desc,
					notes.inserted desc
				limit $2
				offset $3`,
				tag,
