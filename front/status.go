...
func (h *Handler) status(w text.Writer, r *Request, args ...string) {
	...
	if err := h.DB.QueryRowContext(r.Context, `select count(distinct id) from outbox where sent = 0 and attempts < ?`, h.Config.MaxDeliveryAttempts).Scan(&outboxSize); err != nil {
