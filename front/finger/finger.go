...
func (fl *Listener) handle(ctx context.Context, conn net.Conn, wg *sync.WaitGroup) {
	...
	var actor ap.Actor
	if err := fl.DB.QueryRowContext(
		ctx,
		`select actor from persons where actor->>'$.preferredUsername' = ? and host = ?`,
		user,
		fl.Domain,
	...
	rows, err := fl.DB.QueryContext(
		ctx,
		`select object->>'$.content', inserted from notes where public = 1 and author = ? order by inserted desc limit 5`,
		actor.ID,
	)
