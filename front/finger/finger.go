...
func (fl *Listener) handle(ctx context.Context, conn net.Conn) {
	...
	var actor ap.Actor
	if err := fl.DB.QueryRowContext(ctx, `select json(actor) from persons where actor->>'$.preferredUsername' = ? and ed25519privkey is not null`, user).Scan(&actor); err != nil && errors.Is(err, sql.ErrNoRows) {
