	...
	if r.User == nil {
		rows, err = h.DB.QueryContext(
			...
			`
				...
				left join persons groups on
					groups.actor->>'$.type' = 'Group' and (groups.id = notes.object->>'$.audience' or exists (select 1 from shares where shares.by = groups.id and shares.note = notes.id))
				where
					notes.public = 1 and
		...
		rows, err = h.DB.QueryContext(
			...
			`
				...
				left join persons groups on
					groups.actor->>'$.type' = 'Group' and (groups.id = u.object->>'$.audience' or exists (select 1 from shares where shares.by = groups.id and shares.note = u.id))
				group by
					u.id
				order by
					round(u.rank, 1),
