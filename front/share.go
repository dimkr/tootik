...
func (h *Handler) shouldThrottleShare(r *Request) (bool, error) {
	...
	if err := h.DB.QueryRowContext(
		r.Context,
		`select count(*), max(inserted) from
	outbox
	where
		activity->>'$.actor' = $1 and
		sender = $1 and
		(activity->>'$.type' = 'Announce' or activity->>'$.type' = 'Undo') and
		inserted > $2`,
		r.User.ID,
...
func (h *Handler) share(w text.Writer, r *Request, args ...string) {
	...
	if err := h.DB.QueryRowContext(
	 ...
	 where
		...
		public = 1 and
		author != $2 and
		not exists (select 1 from shares where note = notes.id and by = $2)`,
		postID,
		r.User.ID,
