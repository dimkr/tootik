...
func (l *Listener) handleInbox(w http.ResponseWriter, r *http.Request) {
	...
	*/
	...
	/*
		...
		of trusting the sender to pass it as-is, unless the activity carries a proof generated by the origin that is
		still valid
	*/
	...
	if activity.Proof == (ap.Proof{}) || !strings.HasPrefix(activity.Proof.VerificationMethod, "https://"+origin+"/") {
		forwarded = origin != senderOrigin
	} else if _, err := l.verifyProof(r.Context(), activity.Proof, &activity, rawActivity, flags); err != nil {
		slog.Warn("Failed to verify integrity proof", "activity", &activity, "error", err)
