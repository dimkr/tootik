...
func (l *Listener) handleInbox(w http.ResponseWriter, r *http.Request) {
	...
	if err := l.DB.QueryRowContext(r.Context(), `select json(actor), rsaprivkey, ed25519seed from persons where actor->>'$.preferredUsername' = ? and ed25519seed is not null`, receiver).Scan(&actor, &rsaPrivKeyDer, &ed25519Seed); errors.Is(err, sql.ErrNoRows) {
