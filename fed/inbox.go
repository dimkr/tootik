...
func (l *Listener) handleInbox(w http.ResponseWriter, r *http.Request) {
	...
	*/
	...
	if len(activity.Proof.VerificationMethod) > len(origin)+9 && activity.Proof.VerificationMethod[0:8] == "https://" && activity.Proof.VerificationMethod[8:8+len(origin)] == origin && activity.Proof.VerificationMethod[8+len(origin)] == '/' {
		if _, err := l.verifyProof(r.Context(), activity.Proof, &activity, rawActivity, flags); err != nil {
