...
func TestResolve_LocalActor(t *testing.T) {
	...
	actor, err := resolver.ResolveID(context.Background(), key, nobody.ID, 0)
	assert.NoError(err)
...
func TestResolve_FederatedActorOldCacheButOffline(t *testing.T) {
	...
	actor, err = resolver.ResolveID(context.Background(), key, "https://0.0.0.0/user/dan", ap.Offline)
	assert.NoError(err)
...
func TestResolve_FederatedActorWrongIDCached(t *testing.T) {
	...
	actor, err := resolver.ResolveID(context.Background(), key, "https://0.0.0.0/users/dan", 0)
	assert.NoError(err)
	assert.Empty(client.Data)
