...
func (s *sender) send(keys [2]httpsig.Key, req *http.Request) (*http.Response, error) {
	...
	if capabilities&ap.RFC9421Ed25519Signatures == 0 && req.Method == http.MethodPost && rand.Float32() > s.Config.Ed25519Threshold {
		slog.DebugContext(req.Context(), "Randomly enabling RFC9421 with Ed25519", "server", req.URL.Host)
		...
		slog.DebugContext(req.Context(), "Randomly enabling RFC9421 with RSA", "server", req.URL.Host)
	...
	if !ap.GatewayURLRegex.MatchString(urlString) {
		if _, err = s.DB.ExecContext(
			...
			slog.WarnContext(req.Context(), "Failed to record server capabilities", "server", req.URL.Host, "error", err)
		}
	}
