...
func TestProof_Sign(t *testing.T) {
	...
	} else if err := VerifyWithContext(privKey.Public(), a.Proof, a.Context, withProof); err != nil {
		t.Fatalf("Failed to verify proof: %v", err)
	}
}
...
func TestProof_Verify(t *testing.T) {
	...
	if err := VerifyWithContext(ed25519.PublicKey(base58.Decode("6MkrJVnaZkeFzdQyMZu1cgjg7k1pZZ6pvBQ7XJPt4swbTQ2")[2:]), a.Proof, a.Context, raw); err != nil {
		t.Fatalf("Failed to verify proof: %v", err)
	}
}
...
func TestProof_VerifyMLDSA(t *testing.T) {
	...
	if err := VerifyWithContext(pub, a.Proof, a.Context, raw); err != nil {
		t.Fatalf("Failed to verify proof: %v", err)
	}
}
